...
pred.mat <- readRDS(paste0('/ihme/hiv/epp_input/gbd20/soc_dets_run/pred_mat_country_spec.RDS'))[,.(ihme_loc_id, year_var, foi)]
...
array.dt <- list(ihme_loc_id = 'AGO', combos = seq(1:length(seq(0,2,by = 0.05))))
...
if(run.name == 'soc_dets_run_sens'){
  scalar <- data.table()
  for(combo in unique(array.dt$combo)){
    x <- data.table(ihme_loc_id = 'AGO', combo = combo, year_id = c(2000:2022), scale_foi = seq(0,2,by = 0.05)[combo])
    scalar <- rbind(x, scalar)
  }
scalar[,pred := NA]
}
