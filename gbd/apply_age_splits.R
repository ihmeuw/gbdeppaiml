...
##choose one random age group to get births from
birth_dt[,total_births_test := sum(total_births), by = c('year', 'run_num')]
birth_dt <- birth_dt[age_group_id == 2,.(age_group_id = 164, birth_prev = sum(birth_prev), total_births = total_births_test), by = c('year', 'run_num', 'sex_id')]
...
# 
# ##remove birth prev from age group 2
# spec_draw[age_group_id == 2, new_hiv := new_hiv - birth_prev]
# 
...
# print(spec_combined)
# spec_combined[age_group_id == 2 & new_hiv < 0, new_hiv := 0]
# 
# ##################################################################################################################
# ## Format and Output
...
# 
# write.csv(spec_combined[,list(sex_id,year_id,run_num,hiv_deaths, non_hiv_deaths, new_hiv,hiv_births,suscept_pop,total_births,pop_neg,pop_lt200,pop_200to350,pop_gt350,pop_art,age_group_id)],paste0(out_dir,"/temp/",loc,"_ART_data.csv"),row.names=F)
# 
# 
